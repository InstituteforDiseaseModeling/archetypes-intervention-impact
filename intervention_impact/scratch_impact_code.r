...
site_details <- fread("site_details.csv")


lookup_all <- lapply(site_details$name, function(site){
  lookup <- fread(file.path(main_dir, paste0("lookup_table_", site, ".csv")))
  lookup[, site:=site]
  return(lookup)
})
...
lookup_all <- merge(lookup_all, site_details[, list(site=name, cluster, country)], by="site", all=T)
setcolorder(lookup_all, c("site", "cluster", "country", "x_Temporary_Larval_Habitat", "IRS_Coverage", "ITN_Coverage", "ACT_Coverage", "initial", "final"))
...
# 
# 
# ggplot(total_prev[Run_Number==0 & x_Temporary_Larval_Habitat==1000 & year>45], aes(x=year, y=prev)) +
#   geom_line()
