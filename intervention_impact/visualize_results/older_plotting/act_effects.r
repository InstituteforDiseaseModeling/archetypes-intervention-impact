...
# to start: al_cm *only*
impact <- all_impact[al_cm==0 & irs==0]
impact[, al_label:= paste0(al_cm*100, "% Effective Treatment")]
impact[, itn_label:= paste0(itn*100, "% ITN Coverage")]

# prelim plotting
summary_impact <- unique(impact[, list(Site_Name, x_Temporary_Larval_Habitat, int_id, label, al_label, itn_label, itn, irs, al_cm, mean_initial, mean_final, min_final, max_final)])


impact_curves <- ggplot(summary_impact, aes(x=mean_initial, color=itn_label, fill=itn_label)) +
                        geom_abline() +
...
impact[, itn_perc:=itn*100]

change_for_regression <- impact[initial_prev>0]
change_for_regression[, Site_Name:=as.factor(Site_Name)]

impact_reg <- lm(prev_change_perc ~ itn_perc, data=change_for_regression)
impact_reg_with_site_fe <- lm(prev_change_perc ~ itn_perc + Site_Name, data=change_for_regression)

ggplot(change_for_regression, aes(x=itn_perc, y=prev_change_perc)) +
  geom_jitter() + 
